...
`/online`·`/sync` 실시간 동기화는 독립 서비스 `online.205.kr`(repo
`205sla/entry-online`)로 이관되어 CODE 205에서 제거됐다. 운영·Nginx·사용량은 그
저장소에서 관리한다. DB 테이블 `sync_projects`·`sync_usage`는 과거 데이터로
보존만 한다(드롭 안 함).
